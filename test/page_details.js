...
describe('page details', function() {
	it('n3:should parse a person', function() {
		return pageDetails.parse('Vlad Filat', { timeout: 20 * 1000 })
	...
	it('json:should parse a person', function() {
		return pageDetails.parse('Vlad Filat', { timeout: 20 * 1000 }, 'json')
			.then(function(details) {
				assert.equal('person', details.type);
			});
	});
	it('n3:should parse a place: Moscow', function() {
		return pageDetails.parse('Moscow', { timeout: 20 * 1000 })
			.then(function(details) {
				// console.log(details);
	...
	it('json:should parse a place: Moscow', function() {
		return pageDetails.parse('Moscow', { timeout: 20 * 1000 }, 'json')
			.then(function(details) {
				// console.log(details);
				assert.equal('place', details.type);
			});
	...
	it('n3:should parse a group: Ministry of Education (Moldova)', function() {
		return pageDetails.parse('Ministry of Education (Moldova)', { timeout: 20 * 1000 })
			.then(function(details) {
				assert.ok(details);
				// console.log(details);
				// assert.equal('place', details.type);
