...
function resourceNameParser(resource) {
	return decodeURIComponent(resource.replace('http://dbpedia.org/resource/', ''));
}
...
module.exports = function(title) {
	...
	var url = 'http://dbpedia.org/data/' + encodeURI(title) + '.json';
	// console.log(url);
	return request({
			url: url,
			timeout: 8 * 1000,
		...
		.then(function(body) {
			...
			var resourceProp = 'http://dbpedia.org/resource/' + encodeURI(title);
