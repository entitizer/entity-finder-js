	...
	'http://www.w3.org/1999/02/22-rdf-syntax-ns#type': [{
		...
		match: {
			value: [
				...
				new RegExp(/^http:\/\/dbpedia.org\/class\/yago\/SocialGroup\d+$/),
				new RegExp(/^http:\/\/dbpedia.org\/class\/yago\/OccupationalGroup\d+$/)
			]
		},
		prop: 'type',
		value: 'group'
...
module.exports = function(title) {
	...
	var url = 'http://dbpedia.org/data/' + title + '.json';
	// console.log(url);
	return request({
			url: url,
			timeout: 8 * 1000,
		...
		.then(function(body) {
			body = body || {};
			var details = {};
			var resource = body['http://dbpedia.org/resource/' + title] || {};
